...
from utils.textos import PAG3_TEXTO1
from utils.textos import PAG3_TEXTO2
from utils.imagens import abre_imagem
from utils.botoes import botoes_pagina3
from utils.processamento import processamento


def pagina3():
    processamento("Embaralhando as cartas...", "Tudo certo!!")
    ...
    posicoes = [0, 1, 2, 3, 4]
    st.markdown(PAG3_TEXTO1, unsafe_allow_html=True)
    st.markdown(PAG3_TEXTO2, unsafe_allow_html=True)
    st.divider()
        ...
        for posicao, item1, item2, item3 in zip(posicoes, coluna1, coluna2, coluna3):
            with col1:
                if posicao != 4:
                    ...
                    abre_imagem(item1[0])
                    st.divider()
                    st.button("Coluna 1", type="primary", on_click=lambda: botoes_pagina3(1))
            with col2:
                if posicao != 4:
                    abre_imagem(item2[1])
                else:
                    abre_imagem(item2[0])
                    st.divider()
                    st.button("Coluna 2", type="primary", on_click=lambda: botoes_pagina3(2))
            with col3:
                if posicao != 4:
                    abre_imagem(item3[1])
                else:
                    abre_imagem(item3[0])
                    st.divider()
                    st.button("Coluna 3", type="primary", on_click=lambda: botoes_pagina3(3))
